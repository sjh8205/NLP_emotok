...
class Solver(object):
    ...
    def build(self,cuda=True):
        ...
        if self.is_train:
            self.writer = TensorboardWriter(str(self.config.logdir))
    ...
    def save_model(self, epoch):
        ...
        ckpt_path = os.path.join(str(self.config.save_path), '{}.pkl'.format(epoch))
        print('Save parameters to {}'.format(ckpt_path))
        torch.save(self.model.state_dict(), ckpt_path)

    def load_model(self, checkpoint):
        """Load parameters from checkpoint"""
#        print('Load parameters from {}'.format(checkpoint))
        epoch = re.match(r"[0-9]*", os.path.basename(str(checkpoint))).group(0)
        self.epoch_i = int(epoch)
        self.model.load_state_dict(torch.load(checkpoint)) # GPU 
#        self.model.load_state_dict(torch.load(checkpoint,map_location=lambda storage, loc:storage)) # CPU
#        self.model.cpu()
        ...
        for epoch_i in range(self.epoch_i, self.config.n_epoch):
            ...
            for batch_i, (conversations, conversation_length, sentence_length) in enumerate(tqdm(self.train_data_loader, ncols=80)):
                ...
                if batch_i % self.config.print_every == 0:
                    tqdm.write(
                        'Epoch: {}, iter {}: loss = {:.3f}'.format(epoch_i+1,batch_i,batch_loss.item()/n_words.item()))
            ...
            print_str = 'Epoch {} loss average: {:.3f}'.format(epoch_i+1,epoch_loss)
            print(print_str)

            if epoch_i % self.config.save_every_epoch == 0 or epoch_i+1 == self.config.n_epoch:
    ...
    def generate_sentence(self, input_sentences, input_sentence_length,
                          input_conversation_length, target_sentences, on_cpu=False):
        self.model.eval()

        # [batch_size, max_seq_len, vocab_size]
        generated_sentences,final_score = self.model(
            input_sentences,
            input_sentence_length,
            input_conversation_length,
            target_sentences,
        ...
        #print(final_score)
        gen_sen = ''
        for input_sent, output_sent in zip(input_sentences, generated_sentences):
            gen_sen = '\n'.join([self.vocab.decode(sent) for sent in output_sent])
        #print(gen_sen)

        return gen_sen

    def nlg(self, eval_data_loader):
        self.eval_data_loader = eval_data_loader
        #self.model.eval()
        generated_sen = ''
        for batch_i, (conversations, conversation_length, sentence_length) in enumerate(self.eval_data_loader):
#            print('conversations : ',conversations)
#           print('conversation_length : ', conversation_length)
#            print('sentence_length : ', sentence_length)
            target_conversations = [conv[1:] for conv in conversations]
#            print('target_conversations : ', target_conversations)
            # flatten input and target conversations
 ...
 #           print('input_conversation_length : ', input_conversation_length)
 #           print('target_sentence_length : ', target_sentence_length)
 #           print('target_senteces : ', target_sentences)
 #           print('sentence_length : ', sentence_length)
            ...
            if batch_i == 0:
                input_conversations = [conv[:-1] for conv in conversations]
                input_sentences = [sent for conv in input_conversations for sent in conv]
#                print('input_conversations : ', input_conversations)
#                print('input_sentences : ', input_sentences)
                with torch.no_grad():
                    input_sentences = to_var(torch.LongTensor(input_sentences))
                generated_sen = self.generate_sentence(sentences, sentence_length, input_conversation_length, target_sentences)
    ...
    def test(self):
        ...
        print('Number of words: {}'.format(n_total_words))
        print('Bits per word: {:.3f}'.format(epoch_loss))
        word_perplexity = np.exp(epoch_loss)

        print_str = 'Word perplexity : {:.3f}\n'.format(word_perplexity)
        print(print_str)

        return word_perplexity

    def embedding_metric(self, word2vec):
        ...
        for batch_i, (conversations, conversation_length, sentence_length) \
                in enumerate(self.eval_data_loader):
            # conversations: (batch_size) list of conversations
            #   conversation: list of sentences
            #   sentence: list of tokens
            # conversation_length: list of int
        ...
        print_str = 'Metrics - Average: {:.3f}, Extrema: {:.3f}, Greedy: {:.3f}'.format(epoch_average,epoch_extrema,epoch_greedy)
        print(print_str)
        print('\n')
    ...
    def train(self):
        ...
        for epoch_i in range(self.epoch_i, self.config.n_epoch):
            ...
            for batch_i, (conversations, conversation_length, sentence_length) \
                ...
                # reset gradient
                self.optimizer.zero_grad()
                sentence_logits, final_scores, kl_div, _, _ = self.model(
                    sentences,
                    sentence_length,
                    input_conversation_length,
                    target_sentences)
                ...
                if batch_i % self.config.print_every == 0:
                    print_str = 'Epoch: {}, iter {}: loss = {:.3f}, recon = {:.3f}, kl_div = {:.3f}'.format(epoch_i+1,batch_i,batch_loss.item() / n_words.item(), recon_loss.item() / n_words.item(), kl_div.item() / n_words.item())
                    if self.config.bow:
                        print_str += ', bow_loss = {:.3f}'.format(bow_loss.item() / n_words.item())
            ...
            print_str = 'Epoch {} loss average: {:.3f}, recon_loss: {:.3f}, kl_div: {:.3f}'.format(epoch_i+1, epoch_loss, epoch_recon_loss, epoch_kl_div)
            if bow_loss_history:
                self.epoch_bow_loss = np.sum(bow_loss_history) / n_total_words
                print_str += ', bow_loss = {:.3f}'.format(self.epoch_bow_loss)
            print(print_str)
...
#        print(gen_sen)
        ...
        '''
        with open(str(os.path.join(str(self.config.save_path), 'samples.txt')), 'a') as f:
            f.write('<Epoch {}>\n\n'.format(self.epoch_i))
...
#                print('input_sentences : ', input_sentences)
                with torch.no_grad():
                    input_sentences = to_var(torch.LongTensor(input_sentences))
                generated_sen = self.generate_sentence(sentences,
                                       sentence_length,
                                       input_conversation_length,
                                       input_sentences,
                                       target_sentences)
    ...
    def evaluate(self):
        ...
        print_str = 'Validation loss: {:.3f}, recon_loss: {:.3f}, kl_div: {:.3f}'.format(epoch_loss, epoch_recon_loss, epoch_kl_div)
        if bow_loss_history:
            epoch_bow_loss = np.sum(bow_loss_history) / n_total_words
            print_str += ', bow_loss = {:.3f}'.format(epoch_bow_loss)
        print(print_str)
        print('\n')
    ...
    def importance_sample(self):
        ...
        for batch_i, (conversations, conversation_length, sentence_length) \
            ...
            for j in range(self.config.importance_sample):
                sentence_logits, final_scores, kl_div, log_p_z, log_q_zx = self.model(
                    sentences,
                    sentence_length,
                    input_conversation_length,
                    target_sentences)
        ...
        #print('Bits per word: {:.3f}'.format(bits_per_word))
        #word_perplexity = np.exp(bits_per_word)

        #epoch_kl_div = np.sum(kl_div_history) / n_total_words

        #print_str = 'Word perplexity upperbound using {} importance samples: {:.3f}, kl_div: {:.3f}\n'.format(self.config.importance_sample, word_perplexity, epoch_kl_div)
        #print(print_str)
